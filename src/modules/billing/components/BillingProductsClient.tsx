        ...
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="px-4 py-3">
            <div className="grid grid-cols-6 items-center gap-4">
              ...
              <div className="h-4 w-28 animate-pulse rounded bg-slate-200/70" />
              <div className="ml-auto h-6 w-24 animate-pulse rounded bg-slate-200/70" />
            </div>
          </div>
        ))}
      </div>
...
function EmptyState({
  ...
  return variant === "no_match" ? (
    ...
    <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-500">
      <p>No products yet.</p>
      <p className="mt-1">
        Click <span className="font-semibold">New Product</span> to create your
        first one, or <span className="font-semibold">Refresh</span> to sync
        from Stripe.
      </p>
    </div>
  );
...
export default function BillingProductsClient() {
  ...
  const actionDivider = "border-l-2 border-slate-200";
            ...
            <table className="min-w-max w-full border-collapse text-sm">
              <thead className="sticky top-0 z-20 bg-slate-100">
                <tr className="text-left">
                  ...
                  <th
                    className={`${actionThClass} ${actionDivider} sticky z-30 bg-slate-100`}
                ...
                {filtered.map((p) => {
                  ...
                  return (
                    <tr key={p.stripe_product_id} className="hover:bg-slate-50">
                      ...
                      <td
                        className={`${actionTdClass} ${actionDivider} sticky bg-white`}
                        style={{ right: ACTION_COL_W * 2 }}
                      >
                        <Link
                          href={`/billing/products/${encodeURIComponent(p.stripe_product_id)}`}
                          className="inline-flex p-1 !text-slate-600 transition-colors hover:!text-slate-900"
                          title="View"
                        >
                      ...
                      >
                        <Link
                          ...
                          className="inline-flex p-1 !text-indigo-600 transition-colors hover:!text-indigo-700"
                          title="Edit"
                        >
                      ...
                      >
                        {isArchived ? (
                          ...
                          <button
                            ...
                            className="inline-flex cursor-pointer p-1 !text-rose-500 transition-colors hover:!text-rose-600"
                            title="Archive"
                          >
