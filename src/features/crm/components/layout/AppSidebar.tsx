...
const SETTINGS_ROLES = new Set(["manager", "admin"]);
...
function hasRoleAccess(
  ...
  return (
    Array.isArray(roles) &&
    roles.some(
      (r) => typeof r === "string" && allowedRoles.has(r.toLowerCase()),
    )
  );
}
...
export function AppSidebar() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const privilegedAllowed = hasRoleAccess(
          (profile as any)?.role,
          PRIVILEGED_ROLES,
        );
        const settingsAllowed = hasRoleAccess(
          (profile as any)?.role,
          SETTINGS_ROLES,
        );
        ...
        if (!cancelled) {
          ...
          setCanSeeSettings(settingsAllowed);
  ...
  if (workspaceLoading || !permissionsLoaded) {
    return <SidebarSkeleton isDark={isDark} />;
  }
        ...
        {!collapsed && (
          <p>
            ...
            <span className={`font-medium ${footerValue}`}>
              {ctxTeamName ?? effectiveTeamId ?? tDomain("fallbacks.empty")}
            </span>
          </p>
        )}
      </div>
