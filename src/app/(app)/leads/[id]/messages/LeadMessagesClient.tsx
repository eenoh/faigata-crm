...
export function LeadMessagesClient() {
  ...
  useEffect(() => {
    ...
    async function loadLead() {
      ...
      try {
        ...
        ).then((r) => r.json() as Promise<LeadSummary>);
    ...
    async function loadCurrentUser() {
      try {
        ...
        // resolve avatar path → signed URL if needed
        if (profile.avatar_url) {
          if (
            ...
            try {
              ...
              if (signedErr) {
                console.error(
                  "[Messages] createSignedUrl error",
                  signedErr
                );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error("[Messages] Failed to create message", await res.text());
        return;
      }
