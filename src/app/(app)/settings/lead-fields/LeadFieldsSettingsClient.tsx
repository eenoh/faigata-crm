...
export function LeadFieldsSettingsClient() {
  ...
  const teamId = searchParams.get("team"); // ?team=... in URL
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!teamId) {
        console.warn("No teamId in URL, cannot load lead fields");
        setLoading(false);
        return;
      }
  ...
  async function handleSave() {
    if (!teamId) {
      setErrorMessage(
        "Missing team. Please open this page from your workspace URL."
      );
      return;
    }
    ...
    return (
      <div className="max-w-xl rounded-2xl border border-rose-100 bg-rose-50 px-4 py-3 text-sm text-rose-700">
        ...
        <p className="mt-1">
          ...
          in the URL, or use the sidebar navigation from your workspace.
        </p>
      </div>
    );
  }
