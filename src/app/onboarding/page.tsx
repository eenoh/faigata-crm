...
export default function OnboardingPage() {
  ...
  async function handleFinish() {
    try {
      // send fields from onboarding to DB
      await fetch("/api/lead-fields", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fields }), // your CustomFieldRow[]
      });

      // TODO later: send pipeline, metrics, etc.
    } catch (err) {
      ...
      <div className="space-y-4">
        ...
        {props.importFileName && (
          <p className="text-xs text-slate-600">
            Selected file: <span className="font-medium">{props.importFileName}</span>
          </p>
        )}
...
function StepConversionMetrics(props: {
  ...
  function updateMetric(index: number, patch: Partial<ConversionMetricRow>) {
    // guard: index might be out of range if rows were removed
